...
class AssetDef:
    def __init__(self, asset, name, base_position, nature=0) -> None:
        self.asset = asset
        self.name = name
...
class WorldAsset():
    def __init__(self, gym, sim, num_envs, env_origins, device, train_ratio=0.95) -> None:
        ...
        with open('/common/home/dm1487/robotics_research/legged_manipulation/gaited-walk/generate_worlds/worlds.json', 'r') as file:
            INPLAY_ASSETS = json.load(file)

        random.shuffle(INPLAY_ASSETS)
    ...
    def define_world(self, env_id, chosen_asset_inplay_idx):
        """
        ...
        # print('chosen_asset_inplay_idx', chosen_asset_inplay_idx, len(assets_inplay))
        for asset_inplay_idx, i in enumerate(assets_inplay):
            if asset_inplay_idx != chosen_asset_inplay_idx:
                continue
            gym_assets = []
            asset_names = []
            asset_pos = []
            ...
            for idx, name in enumerate(i['name']):
                ...
                asset_nature.append(nature)
            assets_container.append([AssetDef(asset, name, pos, nature) for asset, name, pos, nature in zip(gym_assets, asset_names, asset_pos, asset_nature)])
    ...
    def create_actor(self, env_id, chosen_asset_inplay_idx, env_handle, env_origin):
        """
        ...
        # print(list(range(len(self.inplay_assets))))
        # chosen_asset_inplay_idx = random.choice(range(len(self.inplay_assets)))
        # print('chosen_asset_inplay_idx', chosen_asset_inplay_idx)
        assets_container = self.define_world(env_id, env_id)
        self.env_assets_map[env_id] = assets_container

        for asset_container in assets_container:
            for asset in asset_container:
                pose = gymapi.Transform()
                pos = env_origin.clone(); pos[0] += asset.base_position[0]; pos[1] += asset.base_position[1]; pos[2] = 0.2 
                yaw = asset.base_position[2]
                pose.p = gymapi.Vec3(*pos)
                pose.r = gymapi.Quat(0, 0, np.sin(yaw/2), np.cos(yaw/2))
                ah = self.gym.create_actor(env_handle, asset.asset, pose, asset.name, env_id, 0, 0)
                if asset.nature == 1:
    ...
    def reset_idx(self, env_ids):
                ...
                for asset in asset_container:
                    ...
                    if idx == random_idx:
                        ...
                        if asset.name.startswith('fb_three_mov'):
                            ...
                            base_pos = [*asset.base_position[:2], 0.2]
                            base_positions.append(base_pos)
                            base_orientations.append([0, 0, np.sin(asset.base_position[2]/2), np.cos(asset.base_position[2]/2)])
                        # current_ctr += 1
        ...
        base_orientations = torch.tensor(base_orientations, dtype=torch.float32, device='cuda:0')

        
        env_origins = torch.vstack(env_origins)
        self.all_root_states[actor_indices, :3] = base_positions + env_origins
        self.all_root_states[actor_indices, 3:7] = base_orientations
