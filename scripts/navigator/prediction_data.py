...
def load_policy(env):
    ...
    # model_ckpt = logger.glob('checkpoints/ac_weights_last.pt')
    # random_idx = np.random.randint(0, len(model_ckpts))
    # model_ckpt = model_ckpts[random_idx]
    # print(model_ckpt)
...
def collect_data(env):
    ...
    prediction_obs = env.get_observations()['obs'].clone()
    print(prediction_obs[0, :2])

    return input_obs, target_obs, fsw_obs, prediction_obs
...
if __name__ == "__main__":
    ...
    model_path = '/common/home/dm1487/robotics_research/legged_manipulation/gaited-walk/runs/high_level_policy/2023-05-01/navigator_train/141753.148904'
    ...
    input_obs, target_obs, fsw_obs, prediction_obs = collect_data(env)
    input_data = torch.zeros((env.num_envs, 750, input_obs.shape[-1]), device=env.device)
    prediction_data = torch.zeros((env.num_envs, 750, prediction_obs.shape[-1]), device=env.device)
    ...
    for i in range(num_eval_steps):
        with torch.inference_mode():
            if save:
                ...
                prediction_data[env_idx, env_step, :] = prediction_obs.unsqueeze(1)
                ...
                done_envs = (dones).nonzero().view(-1)
                if done_envs.size(0) > 0:
                    ...
                    q_s[q_id].put({
                        ...
                        'prediction': prediction_data[done_envs].clone().cpu(),
                        })
                    ...
                    prediction_data[done_envs, :, :] = 0.0
                    done_data[done_envs, :] = False
                    pbar.update(1)
                    traj_ctr += done_envs.size(0)
                    print(traj_ctr/pbar.n)

                done_env_horizon = (env_step == 750).nonzero()[:, 0].view(-1)
                if done_env_horizon.size(0) > 0:
                    ...
                    q_s[q_id].put({
                        ...
                        'prediction': prediction_data[done_env_horizon].clone().cpu(),
                        })
                    ...
                    prediction_data[done_env_horizon, :, :] = 0.0
                    done_data[done_env_horizon, :] = False
                    pbar.update(1)
                    # env.reset(done_env_horizon)
                    traj_ctr += done_env_horizon.size(0)
                    print(traj_ctr/pbar.n)
