...
class TransformerDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        target_joint_pos = torch.tensor(data['input'][1:self.sequence_length, 18:30])
        target_joint_vel = torch.tensor(data['input'][1:self.sequence_length, 30:42])
        target_pose = torch.tensor(data['target'][1:self.sequence_length, :6])
        target_priv_info = torch.tensor(data['target'][1:self.sequence_length, 6:])
        target = torch.cat([target_joint_pos, target_joint_vel, target_pose, target_priv_info], dim=-1)
