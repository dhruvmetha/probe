from data import PoseEstimatorDataset, PoseSceneEstimatorDataset
from torch.utils.data import DataLoader
from torch.nn import functional as F
from torch import nn
from torch import optim
from model import ScenePoseTransformer
...
class PoseSceneEstimator:
    def __init__(self, input_size, causal=False, num_layers=2, num_heads=2, device='cuda:0', embed_size=512, hidden_size=2048):
        ...
        self.model = ScenePoseTransformer(input_size=input_size, output_size=24, embed_size=embed_size, hidden_size=hidden_size, num_heads=num_heads, max_sequence_length=self.sequence_length, num_layers=num_layers, estimate_pose=False)
    ...
    def loss_fn(self, out, targ, pose=None):
        ...
        # contact binary
        loss1 = F.binary_cross_entropy(torch.sigmoid(out[:, :, k:k+1]), targ[:, :, k:k+1], reduction='none') + F.binary_cross_entropy(torch.sigmoid(out[:, :, j+7:j+8]), targ[:, :, j+7:j+8], reduction='none') # + F.binary_cross_entropy(torch.sigmoid(out[:, :, k+14:k+15]), targ[:, :, k+14:k+15], reduction='none') 
        loss1 = torch.sum(loss1, dim=-1).unsqueeze(-1)

        # # movable or not movable
        loss2 = F.binary_cross_entropy(torch.sigmoid(out[:, :, k+1:k+2]), targ[:, :, k+1:k+2], reduction='none') + F.binary_cross_entropy(torch.sigmoid(out[:, :, j+8:j+9]), targ[:, :, j+8:j+9], reduction='none') # + F.binary_cross_entropy(torch.sigmoid(out[:, :, k+15:k+16]), targ[:, :, k+15:k+16], reduction='none') # + F.binary_cross_entropy(torch.sigmoid(out[:, :, 22:23]), targ[:, :, 22:23], reduction='none')
        # print(loss2.shape)

        # pose x, y
        loss3 = F.mse_loss(out[:, :, k+2:k+4], targ[:, :, k+2:k+4], reduction='none') + F.mse_loss(out[:, :, j+9:j+11], targ[:, :, j+9:j+11], reduction='none') # + F.mse_loss(out[:, :, k+16:k+18], targ[:, :, k+16:k+18], reduction='none') # + F.mse_loss(out[:, :, 23:25], targ[:, :, 23:25], reduction='none')
        # loss3 = torch.sum(loss3, dim=-1).unsqueeze(-1)

        # # yaw
        loss4 = F.mse_loss(out[:, :, k+4:k+5], targ[:, :, k+4:k+5], reduction='none') + F.mse_loss(out[:, :, j+11:j+12], targ[:, :, j+11:j+12], reduction='none') # + F.mse_loss(out[:, :, j+18:j+19], targ[:, :, j+18:j+19], reduction='none')
        loss4 = torch.sum(loss4, dim=-1).unsqueeze(-1)

        # # width, height
        loss5 = F.mse_loss(out[:, :, k+5:k+7], targ[:, :, k+5:k+7], reduction='none') + F.mse_loss(out[:, :, j+12:j+14], targ[:, :, j+12:j+14], reduction='none') # + F.mse_loss(out[:, :, k+19:k+21], targ[:, :, k+19:k+21], reduction='none')
        ...
        return self.contact_scale * loss1, self.movable_scale * loss2, self.pose_scale * loss3, self.yaw_scale * loss4, self.size_scale * loss5
    ...
    def alternate_loss_fn2(self, pose, pose_targ, out, targ):
        ...
        return loss_pose, loss_object_1, loss_object_2, loss_object_3
    ...
    def train(self, dl, val_dl, save_folder, print_every=50, eval_every=250, save_every=500):
        ...
        for i, (inp, targ, mask, _, pose_targ) in enumerate(dl):
            ...
            pose_targ = (pose_targ[:, :, -2:]).to(self.device)
            pose_out, out = self.model(inp, src_mask=self.src_mask)
            ...
            loss_pose, loss1, loss2, loss3 = self.alternate_loss_fn2(pose_out, pose_targ, out, targ)
            loss_pose = torch.sum(loss_pose*mask)/torch.sum(mask)
            loss1 = torch.sum(loss1*mask)/torch.sum(mask)
            loss2 = torch.sum(loss2*mask)/torch.sum(mask)
            loss3 = torch.sum(loss3*mask)/torch.sum(mask)
            loss = 0.5 * loss_pose + loss1 + 3 * loss2 + 0 * loss3
            ...
            train_loss_pose += 0.5 * loss_pose.item()
            train_loss1 += loss1.item()
            train_loss2 += 3 * loss2.item()
    ...
    def validate(self, dl, save_folder):
        ...
        with torch.inference_mode():
            val_loss = 0
            val_loss_pose, val_loss_1, val_loss_2, val_loss_3, val_loss_4, val_loss_5 = 0, 0, 0, 0, 0, 0
            pbar = tqdm(total=len(dl))
            for i, (inp, targ, mask, fsw, pose_targ) in enumerate(dl):
                ...
                loss_pose, _, _, _ = self.alternate_loss_fn2(pose_out, pose_targ[:, :, -2:], out, targ)
                loss_pose = torch.sum(loss_pose*mask)/torch.sum(mask)
                ...
                # loss_pose, loss1, loss2, loss3 = self.alternate_loss_fn2(pose_out, pose_targ[:, :, -2:], out, targ)
                # loss_pose = torch.sum(loss_pose*mask)/torch.sum(mask)
                # loss1 = torch.sum(loss1*mask)/torch.sum(mask)
                # loss2 = torch.sum(loss2*mask)/torch.sum(mask)
                # loss3 = torch.sum(loss3*mask)/torch.sum(mask)
                ...
                if i < 5:
                    self.save_visualization(inp, targ, out, fsw, pose_targ, pose_targ, mask, vis_path)
        ...
        print(f'vel loss: {val_loss_pose/(i+1)}')
        # self.ious['movable'] = []
        # self.ious['fixed'] = []
        # print("#################")
    ...
    def runner(self, data_folder, save_folder, epochs=100, train_test_split=0.95, train_batch_size=32, val_batch_size=32, learning_rate=1e-4, device='cuda:0', print_every=50, eval_every=250, save_every=500):
        ...
        for epoch in range(epochs):
            ...
            train_ds = PoseSceneEstimatorDataset(files=sub_training_files, sequence_length=self.sequence_length, estimate_pose=True)
            train_dl = DataLoader(train_ds, batch_size=train_batch_size, shuffle=True)
            
            val_ds = PoseSceneEstimatorDataset(files=sub_val_files, sequence_length=self.sequence_length, estimate_pose=True)
...
if __name__ == '__main__':
    ...
    se = PoseSceneEstimator(input_size=input_size, causal=causal, num_layers=num_layers, num_heads=num_heads)
    ...
    with open(f'{save_folder}/info.txt', 'w') as f:
        f.write(f'{"causal" if causal else "not causal"}, {"no" if input_size == 39 else ""} pose, {"yes" if load_pretrained_model else "no"} pretrain, split loss, {num_layers} layers, {num_heads} heads, mse loss, alternate_fn2, all data 1 (equal), with confidence bit')

    se.runner(data_folders, save_folder, epochs=50, train_test_split=0.98, train_batch_size=bs, val_batch_size=bs, learning_rate=1e-4, device='cuda:0', print_every=250, eval_every=500, save_every=500)
